...
bool at_set_device_id(const char **argv, const int argc)
{
    ...
    dev->set_device_id(argv[0], false);
    ei_printf("OK\n");
...
bool at_get_config(void)
{
    ...
    /* build fusion list crashes on Particle, hardcoding sensor string for now */
...
bool at_set_sample_settings(const char **argv, const int argc)
{
    ...
    if(argc >= 4) {
        dev->set_sample_hmac_key(argv[3], false);
    }
...
bool at_set_mgmt_url(const char **argv, const int argc)
{
    ...
    dev->set_management_url(argv[0], false);
...
bool at_set_upload_settings(const char **argv, const int argc)
{
    ...
    dev->set_upload_path(argv[1], false);
...
bool at_set_upload_host(const char **argv, const int argc)
{
    ...
    dev->set_upload_host(argv[0], false);
...
ATServer *ei_at_init(EiDeviceParticle *device)
{
    ATServer *at;
    dev = device;
